...
export default class ActionGamesPage extends BasePage {
  constructor(page) {
    super(page);
    this.topSellersTab = page
      .getByText(CONSTANTS.TABS.TOP_SELLERS, { exact: true })
      .describe('Top Sellers Category Sub-Tab');
    this.discountedGames = page
      .locator(
        'div.ImpressionTrackedElement:has(.HeroCapsuleImageContainer):has(.StoreSalePriceWidgetContainer.Discounted)',
      )
      .describe('Top Sellers Discounted Items List');
  }
  ...
  async clickTopSellersTab() {
    ...
    for (let i = 0; i < CONSTANTS.TIMEOUTS.SCROLL_STEPS; i += 1) {
  ...
  async findGameWithMaxDiscount() {
    ...
    const bestGameData = await this.discountedGames.evaluateAll((elements) => {
      ...
      return {
        ...
        originalPrice: origPriceEl
          ? origPriceEl.textContent.trim().replace(',00', '')
          : '',
        discountedPrice: discPriceEl
          ? discPriceEl.textContent.trim().replace(',00', '')
          : '',
